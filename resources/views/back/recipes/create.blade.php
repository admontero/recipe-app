<x-app-layout>
    <x-slot name="header">
        <h2 class="h4 font-weight-bold">
            {{ __('modules.recipe.create.header') }}
        </h2>
    </x-slot>

    <x-breadcrumbs>
        <li class="breadcrumb-item">
            <a href="{{ route('back.recipes.index') }}">{{ __('modules.recipe.index.header') }}</a>
        </li>

        <li class="breadcrumb-item active" aria-current="page">{{ __('modules.recipe.create.header') }}</li>
    </x-breadcrumbs>

    <x-form-section method="post" action="{{ route('back.recipes.store') }}" id="recipeForm" enctype="multipart/form-data">
        <x-slot name="title">
            {{ __('modules.recipe.create.title') }}
        </x-slot>

        <x-slot name="description">
            {{ __('modules.recipe.create.description') }}
        </x-slot>
        ...
        <x-slot name="form">
            ...
            <div class="mb-2">
                ...
                <input type="hidden" id="quillDescription" name="description" value="{{ old('description') }}"></input>
            ...
            <div class="mb-2">
                ...
                <input type="hidden" id="quillIngredients" name="ingredients" value="{{ old('ingredients') }}"></input>
            ...
            <div class="mb-2">
                ...
                <input type="hidden" id="quillPreparation" name="preparation" value="{{ old('preparation') }}"></input>
    ...
    @push('scripts')
        ...
        <script>
            window.addEventListener('load', function () {
                ...
                tomSelectTags.addOptions(@json(old('tags') ?? []).map(element => ({ title: element, value: element })))
                tomSelectTags.setValue(@json(old('tags') ?? []))
